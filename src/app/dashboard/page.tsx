    ...
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-4 hover:border-white/[0.12] transition-all">
      ...
      <div className="flex items-center gap-1">
        {pos
          ...
          : <ArrowDownRight className="h-3 w-3 text-white/40" />}
...
export default function DashboardPage() {
  ...
  const { revenue, conversions, ads, chartData, topProducts, isLoading, isRefreshing, lastUpdated, refresh, setDateRange: applyRange } = useMetrics({ dateRange, useMock: true });
        ...
        <div className="lg:col-span-3 rounded-xl border border-white/[0.07] bg-white/[0.02] p-5">
          ...
          <div className="mt-3 flex gap-4">
            <span className="flex items-center gap-1.5 text-[10px] text-white/25">
              <span className="h-px w-4 bg-[#00D861]" /> Receita
            </span>
            <span className="flex items-center gap-1.5 text-[10px] text-white/25">
              <span className="h-px w-4 bg-[#5050F2]" /> Leads
            </span>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-5">
          ...
          {isLoading
            ...
            : topProducts.length === 0
              ? <p className="py-8 text-center text-xs text-white/20">Nenhum produto encontrado</p>
